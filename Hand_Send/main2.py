...
while cap.isOpened():
    ...
    if results.multi_hand_landmarks:
        ...
        if thumb_tip.y > index_tip.y:
            if not fist_closed:
                fist_closed = True
                last_hand_x = wrist.x  # Start tracking horizontal position
                screenshot_taken = False
        else:
            if fist_closed:
                # Fist was just released
                fist_closed = False

        # If fist is closed and hand has moved significantly, take screenshot
        if fist_closed and last_hand_x is not None:
            movement = abs(wrist.x - last_hand_x)
            if movement > movement_threshold and not screenshot_taken:
